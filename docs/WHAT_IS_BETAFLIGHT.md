...
horizontally only by tilting**. Want to accelerate east at 3 m/s²? Tilt
east by `atan(3/9.81) ≈ 17°` and the tilted thrust vector does it. That's
...
- **Soft rate PIDs** (p/i/d 20/40/12, yaw 25/45): stock PIDs limit-cycled
  at ~10 Hz on this torque-rich airframe.
...
camera frame (640×360, matching the VADR-TS-002 camera spec) and the race
...
| Track | 60 × 21 m footprint; 3 slots/day + a training cage; manual piloting allowed |
| Gates | 2.70 m outer frame, 1.5 m opening; double gate = ONE gate flown through twice; depth 260 vs 140 mm under review |
| Airframe | 8" Archer Block 2, 8×4.1 props (motors/ESCs/battery/weight TBD); 4 drones/team |
...
| Camera | rolling-shutter Arducam, 1920×1080 @ 60 fps, exposure/gain controllable, Orin-side CLOCK_MONOTONIC timestamps; **intrinsics NOT provided** (we calibrate on-site); IMU pose in the airframe IS provided |
