...
**On the real drone we fly ANGLE mode.** The FC link is MSP at 115200
with attitude back at 20-50 Hz; the organizers' guidance is "keep
anything that has to close a loop at flight rates inside Betaflight".
So the runtime sends tilt angles and throttle (`rc_backend.angle_sticks`,
AUX2 high, `angle_limit` 80) and Betaflight closes attitude. The sim
validates plans in ACRO because the SITL's own attitude filter drifts
in ANGLE mode; `batch_fly --angle` flies the ANGLE shape in the sim
anyway.
...
- **ACRO by default, ANGLE on AUX2 high (`angle_limit` 80), airmode OFF,
  stock rate PIDs 45/80/30 with feedforward 0, `iterm_windup` 30,
  `anti_gravity_gain` 0.** The 2026-08-29 softened
...
context (next expected gate event). The follower uses ground-truth
state through its `StateSource` layer by default;
`AIGP_STATE_SOURCE=deadreckon` swaps in `seeker/dr_estimator.py`
(accel + attitude dead reckoning, baro altitude, a position fix per gate
sighting, its own gate count), the same class the Orin runtime feeds
from the FC.
...
discipline. What does NOT transfer: ground-truth state (the
dead-reckoning estimator replaces it), the exact plant numbers in `[vehicle]`/`[thrust]` (day-1 sysid
...
| Angle mode | sticks command tilt angles; FC self-levels (the runtime's mode on the real drone; the sim validates in acro, see section 4) |
