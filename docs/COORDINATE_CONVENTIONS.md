...
> The pilots cited below (race_pilot etc.) are tape-era and live in
> `src/_archive/`; the MEASURED frame conventions in this doc are still
> the authority and current code (raceline/, solvers/) follows them.
...
5. **CAM**  -  Tech Spec VADR-TS-003 sec.3.7/sec.3.8: 640x360, 20 deg camera uptilt, pinhole `fx=fy=320`, `[cx,cy]=[320,180]`, VFoV 90 deg
...
- **offset_x:**
  ...
  - **CORRECTION (open-loop roll test 2026-06-30, `runtime.roll_test`):** a POSITIVE roll command
    ...
    roll_cmd=-12.53 deg` was therefore a **RIGHT** bank (not "left" as an earlier version stated), which
...
- **offset_y:**
  - `0` = gate at vertical image center (pixel row 180) = a ray **20 deg above horizontal** (because
    the camera is uptilted 20 deg, CAM sec.3.8). So **offset_y = 0 means the gate is ~20 deg ABOVE the
    drone**, not level.
  ...
  - **Level reference** (gate at the drone's own altitude, dead ahead) projects to
    **offset_y ~ +0.65**, derived from CAM: pixel drop `= fy*tan(20 deg) = 320*0.364 = 116.5 px`
    below center; normalized `= 116.5 / 180 = 0.647`. Consistent with LOG-A behavior (below).
  - Evidence: CAM (`fy=320`, `cy=180`, 20 deg uptilt) + LOG-A.
...
- **Race-pilot convention: NEGATIVE `pitch_cmd` = nose-down = FORWARD flight.** In standard NED
  ...
  - Log arithmetic confirms the command (not the resulting direction): both LOG-A and LOG-B show a
    constant `pitch_cmd = -1.15 deg` for the whole run (`-RACE_FWD_LEAN = -0.02 rad = -1.146 deg`).
- [!] **Do not confuse with `PITCH_SIGN = -1.0` (DYN:32).** That sign, and the DYN comment
...
- **`RACE_KP_YAW` must be NEGATIVE to yaw TOWARD the gate.** Derivation: a gate on the RIGHT
  ...
  `-0.03`.**
- [!] **Isolation matters:** yaw sign can't be read by eye while roll is also moving the gate (both
  change `offset_x`). Settle it open-loop (`runtime.yaw_test`), never from a mixed roll+yaw run.
- [ok] **`+offset_x = gate to the RIGHT`**  -  RESOLVED by the open-loop roll test (2026-06-30). LOG-B's
...
- **Positive offset_y = gate BELOW image center = gate is low relative to the drone** (drone is
  ...
  ~20 deg above the drone due to uptilt, and `+0.65` is true level.) Evidence: CAM + `config.py:17`.
...
- [!] **Live config:** `RACE_OFFY_TARGET = 0.0` (`config.py:17`). Since true level ~ `+0.65`,
  targeting `offset_y = 0` aims the drone at a point **~20 deg above** each gate -> it will tend to fly
...
- **Current value: `0.25` rad (`config.py:15`) = `14.32 deg`** (`0.25 * 180/pi = 14.32`).
- Evidence (confirmed live): LOG-B `t=0.291, offset_x=-0.017` would command `-20*-0.017 = +0.34 rad`
  but logs `roll_cmd = +14.32 deg`  -  i.e. clamped exactly at `0.25 rad = 14.32 deg`. [ok]
- Purpose (config comment): cap the roll setpoint so a side blob can't drive an unclamped panic
  bank (BRIEF: an earlier `0.85` shadow caused 33 deg banks into a crash).
...
| `offset_y = 0` | gate ~20 deg ABOVE drone (uptilt) | High | CAM |
...
| `RACE_ROLL_MAX` | `0.25 rad = 14.32 deg` | High (confirmed in LOG-B) | config + LOG-B |
