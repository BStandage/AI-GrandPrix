"""
VENDORED COPY of elodin-sim-aigp/sim/pq_course.py (elodin-free) so the Orin can
load the course without the sim repo. Keep in sync with the sim repo; the sim
itself imports its own copy.
...
_DEFAULT_AIGP = Path(__file__).resolve().parents[2]      # this IS the AI-GrandPrix repo
