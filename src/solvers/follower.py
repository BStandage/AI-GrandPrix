...
COMMIT_LAT_MAX = float(os.environ.get("AIGP_COMMIT_LAT_MAX", "1.4"))    # AIGP_COMMIT_LAT_MAX=0 = no lateral steer once committed: straight on the crossing heading. Was a constant 1.4 # m/s^2 = 8 deg of lean, the plan's own cap. Was 0.6 (3.5 deg): the hairpin g5 arrives at the line still carrying 1.3 m/s of turn, overshot to -0.57 m and touched the frame (sim race_060). Zero when centred, so the straight gates are unaffected.
