"""
Steady pilot v2 params - an attitude-setpoint visual servo. The sim balances; we steer.
...
# =============================================================================================
STEADY_HOVER      = 0.30   # baseline hover thrust GUESS - the integral trim below finds the real one.
...
STEADY_KP_VERT    = 0.16   # thrust per unit of row error. RAISED from 0.10 (run 094047: arrived LOW at
                        #   gate 3, and after a crash the row error alone couldn't lift it off the
                        #   floor) - assertive both ways, per flight feedback.
STEADY_KI_VERT    = 0.02   # INTEGRAL TRIM (thrust/s per unit of row error): learns the true hover
                        #   thrust so the row error goes to ZERO (run 092333 hovered 0.42 of oy high
                        #   without it). ONLY integrates when |vz_est| < STEADY_TRIM_VZ_GATE - i.e. near
...
STEADY_TRIM_VZ_GATE = 0.35 # trim learns only when |vz_est| is below this (m/s) - equilibrium only
STEADY_TRIM_MAX   = 0.06   # clamp on the learned trim (anti-windup; +/-0.06 covers 0.24..0.36 hover)
STEADY_KD_VZ      = 0.03   # thrust per m/s of IMU-integrated climb - quells the bob P-only rows had
STEADY_KD_VZ_COMMIT = 0.10 # stronger vz-arrest INSIDE the commit window: the row servo freezes there,
...
STEADY_KP_COMMIT_SIDE = 0.5 # roll (rad) per unit of aim beyond the deadband, capped at STEADY_ROLL_BLIND
                        #   (3.4 deg) - a drift correction, never a carve, inside a gate's throat
STEADY_THRUST_UP  = 0.18   # climb authority above hover (0.48 ceiling). RAISED from 0.12: post-crash,
...
STEADY_THRUST_DN  = 0.12   # descent authority below hover (0.18 floor - assertive, never a dive)
STEADY_OFFY_BIAS  = -0.06  # aim row offset: hold the opening slightly ABOVE the at-height row = fly a
...
STEADY_CRUISE_UP  = 0.12   # EXTRA row bias while the gate is FAR: hold it LOWER in frame = TRANSIT
...
STEADY_CRUISE_FADE_AREA = 0.05  # the cruise-up bias fades to zero by this blob size (~approach range),
                        #   so ALIGN/COMMIT geometry is completely unchanged
STEADY_VZ_TAU     = 4.0    # leak time-constant (s) of the IMU vertical-velocity integrator
...
# =============================================================================================
STEADY_YAW_SPAWN = 1.694     # the SPAWN FACING in the sim's yaw frame (rad, ~+97 deg) - the heading
...
STEADY_YAW_ON       = 0.25   # yaw only recenters when the aim point drifts beyond this - COARSE FoV
...
STEADY_KP_LAT   = 0.50   # roll (rad) per unit of aim offset_x (saturates the cap at |offx| ~ 0.34).
STEADY_KD_LAT   = 1.10   # roll (rad) per unit/s of aim lateral RATE - the COUNTER-BANG: rolls back
                      #   level (or past it) BEFORE the centre is crossed. Scaled with sqrt(KP).
STEADY_OXRATE_MAX  = 0.30  # clamp on the measured lateral rate - the damping's ceiling. MUST rise with
...
STEADY_OYRATE_GATE = 0.25  # when the aim is moving VERTICALLY faster than this (unit/s - climb or
...
STEADY_ROLL_BLIND  = 0.06  # roll cap (rad ~= 3.4 deg) while lateral-rate samples are being DISCARDED
...
STEADY_CLIMB_GATE  = 0.25  # row error beyond which the forward creep STOPS (the vertical twin of the
...
STEADY_ROLL_MAX = 0.17   # bank cap (rad ~= 10 deg) -> ~1.7 m/s^2 lateral. Assertive, still recoverable
                      #   within the vision lag envelope.
STEADY_ROLL_SLEW = 0.60  # max roll-command slew (rad/s) - the bang AND the counter must both develop
                      #   fast enough to matter (0.30 took 0.57 s just to reach the old cap).
STEADY_YAW_STEP_MAX = 0.03   # cap per frame (~52 deg/s at 30 fps)
STEADY_YAW_MAX_OFF  = 0.95   # anti-windup: stop turning once the aim point is AT the frame edge.
...
# =============================================================================================
STEADY_PITCH_FWD  = 0.05   # forward lean (rad) when aimed
STEADY_AIM_REF    = 0.40   # creep fades to 0 as |offset_x| approaches this (turn first, then close)
STEADY_PITCH_SLEW = 0.15   # max pitch-command slew (rad/s) - eases the creep in/out, no lurches
STEADY_HOLD_KP = 0.15      # STATION-HOLD gain for near-gate climbs (hover_pilot's proven range-hold):
...
STEADY_ALIGN_BRAKE = 0.035 # BACK-pitch (rad) held during ALIGN - zeroing the creep does NOT stop the
...
# =============================================================================================
STEADY_ALIGN_AREA  = 0.10  # blob fraction where a MISALIGNED approach starts braking + strafing (~4-5 m
...
STEADY_COMMIT_AREA = 0.20  # blob fraction where the gate is NEAR: if ALIGNED we commit
STEADY_COMMIT_ALIGN = 0.15 # |aim offset_x| required to actually COMMIT. Run 095624 committed on area
...
STEADY_COMMIT_VZ   = 0.7   # max |vertical speed| (m/s) to ENTER commit. Run 132724 committed while
...
STEADY_COMMIT_ROW  = 0.35  # VERTICAL twin of the alignment requirement: |aim_oy - row target| must be
...
STEADY_PASS_AREA   = 0.30  # arms a pass - ONLY while committed (aligned + close). Area receding from a
                        #   lateral scrape used to count as a "pass" (run 095624 counted 6).
STEADY_PASS_DROP   = 0.7   # counted when area falls to this * peak
STEADY_PASS_REARM  = 0.15  # refractory: no re-arm until the passed gate shrinks below this
...
# =============================================================================================
STEADY_MIN_AREA     = 0.002  # ignore smaller blobs (next gate after a pass reads ~0.0025)
STEADY_TRACK_RADIUS = 0.20   # max in-frame jump to still be the SAME gate
STEADY_TRACK_MAX_MISS = 5    # frames unmatched before re-acquiring
STEADY_HOP_DIST     = 0.25   # bigger tracked jump = lock hop -> reseed the smoother
STEADY_AVOID_RADIUS = 0.30   # post-pass: exclude detections near the flown-through gate
STEADY_AVOID_MIN_AREA = 0.10 # a detection only KEEPS the avoid lock if it is still BIG - the genuinely
...
STEADY_AVOID_S      = 2.5    # hard expiry (s after the pass) - the avoid's whole job is the 1-2 s
                          #   handoff window; a stale marker is only a liability after that.
STEADY_OFF_ALPHA    = 0.4    # EMA on the aim offsets
STEADY_MAX_STALE    = 6      # frames without a fresh measurement before flying HOLD (hover, no steer)
