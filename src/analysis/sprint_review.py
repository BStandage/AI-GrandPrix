"""
Deterministic VISUAL replay of a SPRINT-pilot flight. Feeds the RECORDED session (real camera JPGs,
real per-frame detections, real HIGHRES_IMU stream, real race_status) through the REAL sprint pilot
(pilots.sprint_pilot.update_sprint_control) at the recorded cadence - IMU records drive control
ticks exactly like the live loop, frames land between them - and paints onto each JPG what the
pilot saw and decided:
  ...
  * the estimated artificial horizon (grey line from the IMU attitude filter - if this line is
    wrong, every world-frame bearing is wrong)
  * a panel with the full control state, including the TRANSIT layer: trans state, v_est, the
    range proxy vs the stopping distance (the brake trigger, watchable frame by frame)
  * a pitch bar along the bottom scaled to the SPRINT lean (creep is the small tick near centre)

Because it runs the pilot's own tracker / pass / servo / transit code (not a re-implementation),
the overlay is a faithful reconstruction - and after a code change, re-running it over an OLD
session shows what the NEW code would have decided on the same inputs (open-loop: the trajectory
doesn't change, the decisions do). The pilot's live per-tick CSV (sprint_dbg_*.csv) is the numeric
twin of this view; its first 24 columns are byte-compatible with steady_dbg_*.csv, transit columns
appended.

Usage:
    python -m analysis.sprint_review                        # newest session -> sprint_review.mp4
    python -m analysis.sprint_review <session_dir>
    python -m analysis.sprint_review <session_dir> --png 40 120    # also dump PNGs, seq 40..120
    python -m analysis.sprint_review <session_dir> --no-mp4 --png 40 120
"""
...
import pilots.sprint_pilot.sprint_pilot as ap
from pilots.sprint_pilot.config import SPRINT_LEAN, SPRINT_AVOID_RADIUS
...
def annotate(img, dets, data, t_s, frame_id):
    ...
    _horizon(img, data.get("_sp_est_roll", 0.0), data.get("_sp_est_pitch", 0.0))

    # which det is the tracked one: nearest to the pilot's raw lock position
    lock = data.get("_sp_track_off")
    locked_i = -1
    ...
    if avoid is not None:
        cv2.ellipse(img, _px(*avoid), (int(SPRINT_AVOID_RADIUS * W / 2), int(SPRINT_AVOID_RADIUS * H / 2)),
                    0, 0, 360, RED, 1)

    # smoothed AIM point (the opening, when located) the controller actually reads
    off = data.get("_sp_off_smooth")
    if off is not None:
        cv2.drawMarker(img, _px(*off), CYAN, cv2.MARKER_CROSS, 16, 2)

    heading = data.get("_sp_yaw", 0.0)
    pitch = data.get("_sp_des_pitch", 0.0)
    gc = data.get("_sp_gate_count", 0)
    stale = data.get("_sp_stale", 0)
    ox, oy = data.get("_sp_off", (0.0, 0.0))
    trans = data.get("_sp_trans", "CREEP")
    trans_col = MAGENTA if trans != "CREEP" else WHITE

    _panel(img, [
        (f"t={t_s:6.2f}s  frame {frame_id}  [{data.get('sprint_regime', '?')}]", WHITE),
        (f"gates passed: {gc}", GREEN if gc else WHITE),
        (f"TRANS={trans}  v_est={data.get('_sp_v_est', 0.0):+4.1f} m/s", trans_col),
        (f"rng~{data.get('_sp_rng_est', 0.0):4.1f}m  d_stop={data.get('_sp_d_stop', 0.0):4.1f}m",
         RED if data.get('_sp_rng_est', 99.0) <= data.get('_sp_d_stop', 0.0) else trans_col),
        (f"n_dets={len(dets)}  area={data.get('_sp_area', 0.0):.3f}  stale={stale}",
         RED if stale else WHITE),
        (f"aim ox={ox:+.3f}  oy={oy:+.3f}", WHITE),
        (f"commit={data.get('_sp_commit', 0.0):.0f}", CYAN),
        (f"heading cmd={math.degrees(heading):+6.1f} deg", CYAN),
        (f"pitch cmd={math.degrees(pitch):+4.2f} deg  thr={data.get('_sp_thrust', 0.0):.3f}", WHITE),
        (f"vz_est={data.get('_sp_climb', 0.0):+4.1f}  est r/p="
         f"{math.degrees(data.get('_sp_est_roll', 0.0)):+4.1f}/"
         f"{math.degrees(data.get('_sp_est_pitch', 0.0)):+4.1f}", WHITE),
        (f"pass: armed={int(data.get('_sp_pass_armed', False))} block={int(data.get('_sp_pass_block', False))}",
         WHITE),
    ])

    # pitch bar along the bottom: centre = 0, full half-width = the SPRINT lean (the brake lean
    # overshoots slightly past the left end; the creep is the small deflection near centre)
    by = H - 12
    bx0, half = 10, (W - 20) // 2
    cv2.line(img, (bx0, by), (bx0 + 2 * half, by), GREY, 1)
    cv2.line(img, (bx0 + half, by - 5), (bx0 + half, by + 5), GREY, 1)
    frac = max(-1.0, min(1.0, pitch / SPRINT_LEAN))
    bar_col = MAGENTA if trans != "CREEP" else CYAN
    cv2.line(img, (bx0 + half, by), (bx0 + half + int(frac * half), by), bar_col, 4)
    return img
...
def run(session_dir, make_mp4=True, png_range=None):
    ...
    writer = None
    if make_mp4:
        out_mp4 = os.path.join(session_dir, "sprint_review.mp4")
    ...
    print(f"=== sprint_review: {os.path.basename(session_dir)}  "
    ...
    for t_ns, kind, rec in events:
        if kind == "race":
            ...
            ap.update_sprint_control(conn, 0, data)     # a real control tick, at the recorded cadence
                ...
                if img is not None:
                    ...
                    if png_dir is not None and png_range[0] <= pending["_seq"] <= png_range[1]:
                        cv2.imwrite(os.path.join(
                            png_dir, f"seq{pending['_seq']:04d}_g{data.get('_sp_gate_count', 0)}.png"), img)
                pending = None
