...
real per-frame detections, real HIGHRES_IMU stream, real race_status) through the REAL steady pilot
(pilots.steady_pilot.update_steady_control) at the recorded cadence - IMU records drive control ticks
...
decisions do). The pilot's live per-tick CSV (steady_dbg_*.csv) is the numeric twin of this view.

Usage:
    python -m analysis.steady_review                        # newest session -> steady_review.mp4
    python -m analysis.steady_review <session_dir>
    python -m analysis.steady_review <session_dir> --png 40 120    # also dump PNGs, seq 40..120
    python -m analysis.steady_review <session_dir> --no-mp4 --png 40 120
"""
...
import pilots.steady_pilot.steady_pilot as ap
from pilots.steady_pilot.config import STEADY_PITCH_FWD, STEADY_AVOID_RADIUS
...
def annotate(img, dets, data, t_s, frame_id):
    ...
    if avoid is not None:
        cv2.ellipse(img, _px(*avoid), (int(STEADY_AVOID_RADIUS * W / 2), int(STEADY_AVOID_RADIUS * H / 2)),
                    0, 0, 360, RED, 1)
    ...
    _panel(img, [
        (f"t={t_s:6.2f}s  frame {frame_id}  [{data.get('steady_regime', '?')}]", WHITE),
    ...
    frac = max(-1.0, min(1.0, pitch / STEADY_PITCH_FWD))
...
def run(session_dir, make_mp4=True, png_range=None):
    ...
    writer = None
    if make_mp4:
        out_mp4 = os.path.join(session_dir, "steady_review.mp4")
    ...
    print(f"=== steady_review: {os.path.basename(session_dir)}  "
    ...
    for t_ns, kind, rec in events:
        if kind == "race":
            ...
            ap.update_steady_control(conn, 0, data)     # a real control tick, at the recorded cadence
