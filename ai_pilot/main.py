...
controller.arm()
from controller import CONTROL_MODE
...
if CONTROL_MODE == "trajectory":
    ...
    print("  PURSUIT: flying the course off ground-truth gate geometry (intercept guidance).", flush=True)
    print("  Chases the active gate, descends to arrive at its height, punches through.", flush=True)
    print("  Be ready to press ESC to abort.", flush=True)
    ...
    print("  When done: python analyze_performance.py", flush=True)
else:
